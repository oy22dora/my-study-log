...
function renderEntries() {
  ...
  if (entries.length > 0 && filteredEntries.length === 0) {
    ...
    emptyState.innerHTML = `
      ...
      <span>今日の集中度と学習メモを保存すると、ここから見返せます。</span>
    `;
  }
...
function renderBiorhythm() {
  if (entries.length === 0) {
    ...
    biorhythmMessage.textContent = "集中度を記録すると表示されます";
...
function getBiorhythmMessage(trend, average, entryLength) {
  ...
  if (trend >= 0.5) {
    return "集中度が少し上がっています";
  }

  if (trend <= -2) {
    return "集中しにくい日が続いています";
  }

  if (trend <= -0.5) {
    return "集中度が少し下がっています";
  }
